...
registerProviderPathResolver(provider, 'config-directory', claudeConfigDirectory);

registerProviderRuntimeFileTransformer(provider, 'memory-session-hook', ({ exists, content, context, filePath }) => {
  const hook = context as MemorySessionHookRegistration;
  const parsed: unknown = exists ? JSON.parse(content) : {};
...
registerProviderTraceReader(provider, 'claude-home', newestClaudeTranscript);
registerProviderArchivePlanner(provider, 'transcript-markdown', planTranscriptArchive);
registerProviderContinuationRotationPlanner(provider, 'cold-resume', planContinuationRotation);
...
registerProviderRuntimeContract(provider, {
  managedFiles: provided([
    {
      ...
      transformer: 'memory-session-hook',
      read: 'text-if-present',
      write: 'direct-replace',
    },
  ]),
  archives: provided({ trigger: 'pre-compact', owner: 'core', planner: 'transcript-markdown' }),
  ...
  traceReaders: provided([{ id: 'claude-home', reader: 'claude-home' }]),
  attachments: provided({ initial: 'text-only', followUp: 'text-only' }),
  events: ['activity', 'init', 'text', 'result', 'error', 'progress'],
  textDelivery: 'mid-turn-complete',
  lifecycle: {
    memory: provided('native-session-hook'),
    compaction: provided('provider-hook'),
    continuation: provided('opaque-token'),
    exchangeCompletion: waived('Claude archives its SDK transcript during compaction'),
  },
  commands: {
    formatting: 'native',
