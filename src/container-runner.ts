...
export async function resolveProviderContribution(
  ...
  if (contract.legacyHostAdapter.kind === 'provided' && contract.legacyHostAdapter.value === 'required' && !fn) {
...
export async function buildMounts(
  ...
  const projectDocument = contract?.projectDocument.kind === 'provided' ? contract.projectDocument.value : undefined;
