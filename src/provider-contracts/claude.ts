...
registerProviderFileTransformer('claude', 'claude-memory-settings', claudeSettingsTransformer);
...
export const CLAUDE_COMPATIBLE_HOST_SURFACES = {
  projectDocument: provided({
    ...
    sourceProtection: provided('install-surface'),
  }),
  stateVolumes: [
    {
      id: 'claude-home',
  ...
  skillBackings: [
    {
      ...
      conflictDiagnostics: provided('warn'),
      templateCopies: provided('in-place'),
    },
  ],
  skillViews: [
    {
  ...
  files: [
    {
      ...
      reconcile: provided({
        transformer: 'claude-memory-settings',
        when: 'group-init',
...
registerProviderHostContract('claude', {
  ...
  // src/providers/claude.ts adapter remains the custom-endpoint overlay.
  environment: waived(
    'base Claude contributes no host environment; the conditional custom-endpoint adapter is unchanged',
  ),
  legacyHostAdapter: waived('the custom-endpoint adapter remains conditionally imported'),
  commands: {
