...
export function protectedProviderDocumentSourcePaths(projectRoot: string): string[] {
  ...
  return listProviderHostContracts().flatMap((contract) => {
    if (contract.projectDocument.kind === 'waived') return [];
    const { baseDocumentFile, sourceProtection } = contract.projectDocument.value;
    return sourceProtection.kind === 'provided' ? [resolveWithinRoot(documentsRoot, baseDocumentFile)] : [];
  });
}

export function providerProjectDocSpec(contract: ProviderHostContract): ProjectDocSpec | undefined {
  if (contract.projectDocument.kind === 'waived') return undefined;
  const { fileName, baseDocumentFile, extraSections, maxBytes } = contract.projectDocument.value;
    ...
    if (operation.kind === 'state-volume') {
      ...
      if (backing.sharedLinks.kind === 'provided') {
        syncSharedSkillLinks(
          skillsPath,
          selectedSkills,
          backing.conflictDiagnostics.kind === 'provided' && backing.conflictDiagnostics.value === 'warn',
        );
      }
      if (backing.templateCopies.kind === 'provided' && backing.templateCopies.value === 'copy') {
...
function initializeFile(
  ...
  if (file.reconcile.kind === 'waived' || file.reconcile.value.when !== 'group-init') return;
  const transformerProvider = file.reconcile.value.transformerProvider ?? provider;
  const transformer = getProviderFileTransformer(transformerProvider, file.reconcile.value.transformer);
  if (!transformer) {
    throw new Error(
      `Provider '${provider}' file '${file.id}' references missing transformer '${file.reconcile.value.transformer}'`,
    );
  }
  try {
